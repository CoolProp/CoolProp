...
* **Incompressible fluids: Chebyshev caloric fits.** Density and heat capacity now ship as Chebyshev expansions (``density_cheb``/``specific_heat_cheb`` in the fluid JSON, preferred by the loader; the polynomial entries remain as fallback). Enthalpy and entropy are derived from the cp expansion **once at load time** with exact antiderivative recurrences — ``dh/dT == cp`` and ``ds/dT == cp/T`` hold by construction, the historical ``T == Tbase`` singularity class cannot occur in this basis, and the fits are validated positive over each fluid's whole (T, x) domain. Fit degrees are selected by generalized cross-validation (rounded near-linear tables get low order; strongly curved data like the ice-slurry cp keeps high order). Also new: ``add_fluids_as_JSON("INCOMP", ...)`` registers incompressible fluids at runtime (issue `#2384`), and the Desktop GUI gained an "Add incompressible fluid" dialog — paste a property table in SI units, review the fit quality, and the fluid becomes available as ``INCOMP::<name>`` (persisted across restarts).
