...
* **Region 2** — superheated steam and high-:math:`T` gas
  (:math:`T \ge T_\mathrm{sat}(p)` up to :math:`T = 1073.15` K, plus
  the slab :math:`T > 1073.15`).  Backward Gibbs :math:`g_2(T, p)`.
...
implement it (currently ``IF97`` and the tabular backends).
``Web/scripts/fluid_properties.IF97Profile.py`` regenerates the figure
against your local hardware and CoolProp build; the numbers shipped
with the docs come from the GitHub-hosted CI runner.
