#!/usr/bin/env python3
"""
Verify that each fluid's stored SUPERANCILLARY.eos_hash still matches the hash
of its current EOS[0] (with the SUPERANCILLARY subtree excluded).

The `eos_hash` was stamped at SA-generation time by fastchebpure (emitted as
`source_eos_hash` in its output/*_exps.json) and copied verbatim by
dev/scripts/inject_superanc_check_points.py. If anyone has edited the
fluid's EOS since (gas constant, alpha0/alphar coefficients, reducing
state, ...), the current hash no longer matches, and the SA is stale —
bump the fastchebpure pin to regenerate.

This check is a thin Python twin of the C++ test
"Superancillary eos_hash matches current EOS at bit level"; both are wired
to the same byte-stream contract documented in
...
def main():
    ...
    print(f'Checked {checked} fluids with stored eos_hash; skipped {skipped}.')
    if stale:
        ...
        return 1
    print('All superancillary eos_hash stamps match the current EOS.')
    return 0
