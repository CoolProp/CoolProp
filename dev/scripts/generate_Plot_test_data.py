...
if __name__ == '__main__':
    ...
    # Re-format the file so the generated block matches the repo's
    # .clang-format style (CoolProp-2uw.12). Without this the autogenerated
    # output would re-introduce clang-format violations on every regen.
    import shutil, subprocess
    clang_format = shutil.which('clang-format') or shutil.which('clang-format-18')
    if clang_format is None:
        print('warning: clang-format not on PATH; CoolPropPlot.cpp may need manual formatting', file=sys.stderr)
    else:
        subprocess.run([clang_format, '-i', '-style=file', '-fallback-style=none', str(file_path)], check=True)