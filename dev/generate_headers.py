...
def TO_CPP(root_dir, hashes):
    ...
    for infile, outfile, variable in zvalues:
        ...
        # Convert each byte to hex, joined into a comma-separated string.
        # NOTE: unlike the text-JSON headers below, we deliberately do NOT
        # append a trailing 0x00 here.  This blob is raw binary CBOR decoded
        # via nlohmann::json::from_cbor(data, data + size), whose strict mode
        # rejects trailing bytes ("expected end of input").  A NUL terminator
        # would inflate sizeof()/gSize by one and break the decode.

        try:
            h = ["0x{:02x}".format(ord(b)) for b in json_]
        except TypeError:
            h = ["0x{:02x}".format(int(b)) for b in json_]
...
def combine_json(root_dir):
    ...
    depfluids = DependencyManager(destination=os.path.join(root_dir, 'dev', 'all_fluids.cbor'),
