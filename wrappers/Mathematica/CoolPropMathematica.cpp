...
extern "C" DLLEXPORT int PropsSI( WolframLibraryData libData, mint Argc, MArgument *Args, MArgument Res) {
    ...
    // PropsS version takes all strings, not single-character inputs
	double val = PropsSI(outstring,In1string,(double)In1val,In2string,(double)In2val,Fluidstring);
	MArgument_setReal(Res, val);
	return LIBRARY_NO_ERROR;
}

extern "C" DLLEXPORT int HumidAirProps( WolframLibraryData libData, mint Argc, MArgument *Args, MArgument Res) {
	...
	double val = HAProps(outstring,In1string,(double)In1val,In2string,(double)In2val,In3string,(double)In3val);
	MArgument_setReal(Res, val);
	return LIBRARY_NO_ERROR;
}
