...
def params_constants(enum_key):
    ...
    if entries.find("/*") > -1:
        raise ValueError(
            "/* */ style comments are not allowed, replace them with // style comments"
        )
    if not entries:
        raise ValueError("Unable to find " + enum_key)
...
def config_constants():
    ...
    matching_lines = [
        ...
        if "#define CONFIGURATION_KEYS_ENUM" in line
    ]
...
def generate():
    data = [
        (enum, params_constants(enum))
        for enum in ["parameters", "input_pairs", "fluid_types", "phases"]
    ]
