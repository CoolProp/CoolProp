...
def params_constants(enum_key):
    fName = os.path.join('..', '..', 'include', 'CoolProp', 'DataStructures.h')
...
def config_constants():
    ...
    keys = []
    while iline < len(contents) and (contents[iline].strip().startswith('X(') or contents[iline].strip().startswith('"')):
...
def generate_cython(data, config_data):
    ...
    with open('CoolProp/constants_header.pxd', 'w') as pxd_output_file:
    ...
    with open('CoolProp/_constants.pyx', 'w') as pyx_output_file:
    ...
    with open('CoolProp/constants.py', 'w') as py_output_file:
