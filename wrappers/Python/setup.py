...
def copy_files():
    ...
    for jsonfile in glob.glob(os.path.join("CoolProp", "include", "*_JSON.h")):
        ...
        os.remove(jsonfile)
    copytree(
        os.path.join(CProot, "externals/fmtlib/fmt"),
        os.path.join("CoolProp", "include", "fmt"),
    )  # Should be deprecated
    # copytree(os.path.join(CProot, 'externals/fmtlib/include/fmt'), os.path.join('CoolProp','include','fmt'))
    copy2(
        os.path.join(CProot, "CoolPropBibTeXLibrary.bib"),
        os.path.join("CoolProp", "CoolPropBibTeXLibrary.bib"),
    )
    print("files copied.")
...
if __name__ == "__main__":
    ...
    if USING_CMAKE:
        ...
        if cmake_compiler == "vc9":
            ...
            if cmake_bitness == "32":
                ...
                raise ValueError(
                    "cmake_bitness must be either 32 or 64; got " + cmake_bitness
                )
        ...
        cmake_build_dir = os.path.join(
            "cmake_build",
            "{compiler}-{bitness}bit".format(
                compiler=cmake_compiler, bitness=cmake_bitness
            ),
        )
        if not os.path.exists(cmake_build_dir):
            os.makedirs(cmake_build_dir)

        cmake_call_string = " ".join(
            [
                "cmake",
                "../../../..",
        ...
        if len(static_libs) != 1:
            raise ValueError(
                "Found more than one static library using CMake build.  Found: "
                + str(static_libs)
            )
        else:
    ...
    if USE_CYTHON:
        ...
        try:
            ...
            raise ImportError(
                "Cython not found, please install it.  You can do a pip install Cython"
            )
        ...
        if parse_version(Cython.__version__) < parse_version("0.20"):
            raise ImportError(
                "Your version of Cython (%s) must be >= 0.20 .  Please update your version of cython"
                % (Cython.__version__,)
            )
        ...
        if parse_version(Cython.__version__) >= parse_version("0.26"):
            cython_directives = dict(profile=_profiling_enabled, embedsignature=True)
        else:
            cython_directives = dict(profile=_profiling_enabled, embedsignature=True)
    else:
        cython_directives = {}
    ...
    if pypi:
        ...
        if os.path.exists(os.path.join("..", "..", "CMakeLists.txt")):
            ...
            raise ValueError(
                "Could not run script from this folder("
                + os.path.abspath(os.path.curdir)
                + "). Run from wrappers/Python folder"
            )
    ...
    common_args = dict(include_dirs=include_dirs, language="c++")

    if USE_CYTHON:
        common_args.update(
            dict(cython_c_in_temp=True, cython_directives=cython_directives)
        )

    if STATIC_LIBRARY_BUILT == True:
        CoolProp_module = Extension(
            "CoolProp.CoolProp",
    ...
    try:
        setup(
            ...
            package_data={
                "CoolProp": ["*.pxd", "CoolPropBibTeXLibrary.bib", "Plots/psyrc"]
                + recursive_collect_includes()
            },
            classifiers=[
