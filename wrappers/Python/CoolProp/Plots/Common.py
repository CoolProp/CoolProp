...
def get_critical_point(state):
    ...
    try:
        ...
        try:
            for crit_state_tmp in state.all_critical_points():
                if crit_state_tmp.stable and (
                    crit_state_tmp.T > crit_state.T or not np.isfinite(crit_state.T)
                ):
    ...
    if len(masses) > 1:
        new_state.set_mass_fractions(
            masses
        )  # Uses mass fraction to work with incompressibles
...
def _get_index(prop):
    if is_string(prop):
        ...
        raise ValueError(
            "Invalid input, expected a string or an int, not {0:s}.".format(str(prop))
        )
...
class BaseQuantity(object):
    ...
    @off_SI.setter
    def off_SI(self, value):
        self._off_SI = value

    def from_SI(self, value):
        return ((value + self.off_SI) * self.mul_SI) + self.add_SI

    def to_SI(self, value):
        return (value - self.add_SI) / self.mul_SI - self.off_SI
...
class BaseDimension(BaseQuantity):
    ...
    def __init__(
        self, add_SI=0.0, mul_SI=1.0, off_SI=0.0, label="", symbol="", unit=""
    ):
        self._label = label
        self._symbol = symbol
        self._unit = unit
...
class Base2DObject(with_metaclass(ABCMeta), object):
    ...
    PLOTS = {"TS": TS, "PH": PH, "HS": HS, "PS": PS, "PD": PD, "TD": TD, "PT": PT}
    ...
    def _get_sat_bounds(self, kind, smin=None, smax=None):
        ...
        try:
            ...
            T_min = T_triple
        self._state.update(
            CoolProp.QT_INPUTS, 0, max([T_triple, T_min]) + self._T_small
        )
        ...
        if kind == CoolProp.iP:
            ...
            raise ValueError(
                "Saturation boundaries have to be defined in T or P, but not in {0:s}".format(
                    str(kind)
                )
            )
...
class IsoLine(Base2DObject):
    ...
    XY_SWITCH = {
        CoolProp.iDmass: {
            Base2DObject.TS: True,
            Base2DObject.PH: True,
            Base2DObject.HS: False,
            Base2DObject.PS: True,
            Base2DObject.PD: None,
            Base2DObject.TD: None,
            Base2DObject.PT: False,
        },
        CoolProp.iHmass: {
            Base2DObject.TS: False,
            Base2DObject.PH: None,
            Base2DObject.HS: None,
            Base2DObject.PS: True,
            Base2DObject.PD: True,
            Base2DObject.TD: False,
            Base2DObject.PT: False,
        },
        CoolProp.iP: {
            Base2DObject.TS: False,
            Base2DObject.PH: None,
            Base2DObject.HS: False,
            Base2DObject.PS: None,
            Base2DObject.PD: None,
            Base2DObject.TD: False,
            Base2DObject.PT: None,
        },
        CoolProp.iSmass: {
            Base2DObject.TS: None,
            Base2DObject.PH: True,
            Base2DObject.HS: None,
            Base2DObject.PS: None,
            Base2DObject.PD: True,
            Base2DObject.TD: False,
            Base2DObject.PT: True,
        },
        CoolProp.iT: {
            Base2DObject.TS: None,
            Base2DObject.PH: True,
            Base2DObject.HS: False,
            Base2DObject.PS: False,
            Base2DObject.PD: False,
            Base2DObject.TD: None,
            Base2DObject.PT: None,
        },
        CoolProp.iQ: {
            Base2DObject.TS: True,
            Base2DObject.PH: True,
            Base2DObject.HS: True,
            Base2DObject.PS: True,
            Base2DObject.PD: True,
            Base2DObject.TD: True,
            Base2DObject.PT: False,
        },
    }
    ...
    def get_update_pair(self):
        ...
        if switch is None:
            ...
            pair, out1, _ = CP.generate_update_pair(
                self.i_index, 0.0, self.x_index, 1.0
            )
        elif switch is True:
            pair, out1, _ = CP.generate_update_pair(
                self.i_index, 0.0, self.y_index, 1.0
            )
        else:
    ...
    def calc_sat_range(self, Trange=None, Prange=None, num=200):
        ...
        try:
            ...
            warnings.warn(
                "An error occurred for the critical inputs, skipping it.", UserWarning
            )
            xcrit = np.NaN
            ycrit = np.NaN
    ...
    def calc_range(self, xvals=None, yvals=None):
        ...
        idxs = [
            v
            for (_, v) in sorted(zip(order, [self.i_index, self.x_index, self.y_index]))
        ]
        vals = [
            v for (_, v) in sorted(zip(order, [np.array(self.value), xvals, yvals]))
        ]
            ...
            try:
                if use_guesses:
                    if np.isfinite(guesses.rhomolar):
                        self.state.update_with_guesses(
                            pair, vals[0][index], vals[1][index], guesses
                        )
                    else:
...
class BasePlot(Base2DObject):
    ...
    UNIT_SYSTEMS = {"SI": SIunits(), "KSI": KSIunits(), "EUR": EURunits()}
    ...
    def __init__(
        self, fluid_ref, graph_type, unit_system="KSI", tp_limits="DEF", **kwargs
    ):
        ...
        if graph_type not in Base2DObject.PLOTS:
            raise ValueError(
                "Invalid graph_type input, expected a string from {0:s}".format(
                    str(self.PLOTS)
                )
            )
    ...
    def system(self, value):
        ...
        if value in self.UNIT_SYSTEMS:
            ...
            raise ValueError(
                "Invalid input, expected a string from {0:s}".format(
    ...
    def limits(self, value):
        ...
        if value in self.TP_LIMITS:
            ...
            raise ValueError(
                "Invalid input, expected a list with 4 items or a string from {0:s}".format(
                    str(self.TP_LIMITS.keys())
                )
            )
...
consider replacing it with "_get_sat_bounds".',
    ...
    def get_axis_limits(self, x_index=None, y_index=None):
        ...
        if (
            ...
            or y_index != self.y_index
            or self.axis.get_autoscalex_on()
            or self.axis.get_autoscaley_on()
        ):
        ...
        if len(xv) == len(yv) and len(yv) > 1:  # assure same length
            if len(xv) == len(yv) and len(yv) == 2:  # only two points
                if np.min(xv) < x < np.max(xv):
                    ...
                    raise ValueError(
                        "Your coordinate has to be between the input values."
                    )
            else:
                ...
                if (
                    xv[index] < x < xv[index + 1]  # nearest below, positive inclination
                    or xv[index] > x > xv[index + 1]
                ):  # nearest above, negative inclination
                    if diff[index] < limit:
                        index = [index - 1, index + 1]
                    else:
                        index = [index, index + 1]
                elif (
                    xv[index - 1] < x < xv[index]  # nearest above, positive inclination
                    or xv[index - 1] > x > xv[index]
                ):  # nearest below, negative inclination
                    if diff[index] < limit:
                        index = [index - 1, index + 1]
                    else:
                        index = [index - 1, index]
                xvnew = xv[index]
                yvnew = yv[index]
                return BasePlot.get_x_y_dydx(
                    xvnew, yvnew, x
                )  # Allow for a single recursion
        else:
            raise ValueError(
                "You have to provide the same amount of x- and y-pairs with at least two entries each."
            )
