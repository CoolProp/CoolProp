...
class PropertyPlot(BasePlot):
    ...
    def calc_isolines(
        self, iso_type=None, iso_range=None, num=15, rounding=False, points=250
    ):
        ...
        if iso_type is None or iso_type == "all":
            for i_type in IsoLine.XY_SWITCH:
                if (
                    IsoLine.XY_SWITCH[i_type].get(
                        self.y_index * 10 + self.x_index, None
                    )
                    is not None
                ):
        ...
        if len(iso_range) <= 1 and num != 1:
            raise ValueError(
                "You have to provide two values for the iso_range, {0} is not valid.".format(
                    iso_range
                )
            )
        ...
        if len(iso_range) == 2 and (num is None or num < 2):
            raise ValueError(
                "Please specify the number of isoline you want e.g. num=10."
            )
            ...
            for line in self.isolines[i]:
                if line.i_index == CoolProp.iQ:
                    if line.value == 0.0 or line.value == 1.0:
                        self.axis.plot(
                            dimx.from_SI(line.x), dimy.from_SI(line.y), **sat_props
                        )
                    else:
    ...
    def draw_process(self, statecontainer, points=None, line_opts=None):
        ...
        """
        warnings.warn(
            'You called the function "draw_process", which is not tested.', UserWarning
        )
...
def InlineLabel(xv, yv, x=None, y=None, axis=None, fig=None):
    warnings.warn(
        'You called the deprecated function "InlineLabel", use "BasePlot.inline_label".',
        DeprecationWarning,
    )
...
class PropsPlot(PropertyPlot):
    def __init__(
        self, fluid_name, graph_type, units="KSI", reciprocal_density=False, **kwargs
    ):
        super(PropsPlot, self).__init__(
            fluid_name,
            graph_type,
            unit_system=units,
        ...
        warnings.warn(
            'You called the deprecated class "PropsPlot", use "PropertyPlot".',
            DeprecationWarning,
        )
...
if __name__ == "__main__":
    plot = PropertyPlot(
        "HEOS::n-Pentane", "PD", unit_system="EUR"
    )  # , reciprocal_density=True)
