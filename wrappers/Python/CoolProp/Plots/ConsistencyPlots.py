...
implemented_solvers = [
    pair for pair in all_solvers if pair not in not_implemented_solvers
]

param_labels = dict(
    Hmolar="Enthalpy [J/mol]/1000",
...
class ConsistencyFigure(object):
    def __init__(
        ...
        additional_skips=[],
        mole_fractions=None,
        p_limits_1phase=None,
        T_limits_1phase=None,
        NT_1phase=40,
        Np_1phase=40,
        NT_2phase=20,
        NQ_2phase=20,
        ...
        for row in self.axes:
            for ax in row:
                ...
                kwargs = dict(
                    p_limits_1phase=p_limits_1phase,
                    T_limits_1phase=T_limits_1phase,
                    NT_1phase=NT_1phase,
                    Np_1phase=Np_1phase,
                    NT_2phase=NT_2phase,
                    NQ_2phase=NQ_2phase,
                ...
                self.axes_list.append(
                    ConsistencyAxis(
                        ax, self, pair, self.fluid, self.backend, *states, **kwargs
                    )
                )
                ax.set_title(pair)
    ...
    def calc_saturation_curves(self):
        ...
        for Q, dic in zip([0, 1], [self.dictL, self.dictV]):
            ...
            for _T in np.logspace(
                np.log10(HEOS.keyed_output(CP.iT_triple)),
                np.log10(HEOS.keyed_output(CP.iT_critical)),
                500,
            ):
                try:
                    HEOS.update(CP.QT_INPUTS, Q, _T)
                    if HEOS.p() < 0:
                        raise ValueError("P is negative:" + str(HEOS.p()))
                    ...
                    print(
                        ...
                        "{T:0.16g}".format(T=_T),
                        "T/Tc:",
                        _T / HEOS.keyed_output(CP.iT_critical),
                    )
    ...
    def calc_Tmax_curve(self):
        ...
        for _p in np.logspace(
            np.log10(HEOS.keyed_output(CP.iP_min) * 1.01),
            np.log10(HEOS.keyed_output(CP.iP_max)),
            300,
        ):
            try:
    ...
    def calc_melting_curve(self):
        ...
        if state.has_melting_line():
            pmelt_min = (
                max(
                    state.melting_line(CP.iP_min, -1, -1),
                    state.keyed_output(CP.iP_triple),
                )
                * 1.01
            )
            pmelt_max = (
                min(
                    state.melting_line(CP.iP_max, -1, -1), state.keyed_output(CP.iP_max)
                )
                * 0.99
            )
        ...
        for p in np.logspace(np.log10(p_min), np.log10(p_max), self.Np_1phase):
            ...
            if self.T_limits_1phase is None:
                ...
                if self.state.has_melting_line():
                    ...
                    T0 = Tmin + 1.1
                Tvec = np.linspace(
                    T0, self.state.keyed_output(CP.iT_max), self.NT_1phase
                )
            else:
                # Use the provided limits for T
                Tvec = np.linspace(
                    self.T_limits_1phase[0], self.T_limits_1phase[1], self.NT_1phase
                )
            ...
            for T in Tvec:
                ...
                try:
                    self.state.update(
                        pairkey,
                        self.state_PT.keyed_output(key1),
                        self.state_PT.keyed_output(key2),
                    )
                except ValueError as VE:
                    print(
                        "update(1p)",
                        ...
                        T,
                        "D",
                        self.state_PT.keyed_output(CP.iDmolar),
                        "{0:18.16g}, {1:18.16g}".format(
                            self.state_PT.keyed_output(key1),
                            self.state_PT.keyed_output(key2),
                        ),
                        VE,
                ...
                if _exception:
                    ...
                    if (
                        abs(self.state_PT.rhomolar() / self.state.rhomolar() - 1) < 1e-3
                        and abs(self.state_PT.p() / self.state.p() - 1) < 1e-3
                        and abs(self.state_PT.T() - self.state.T()) < 1e-3
                    ):
                        xgood.append(x)
                        ygood.append(y)
                        ...
                        print(
                            ...
                            self.state_PT.T(),
                            "Drho:",
                            abs(self.state_PT.rhomolar() / self.state.rhomolar() - 1),
                            abs(self.state_PT.p() / self.state.p() - 1),
                            "DT:",
                            abs(self.state_PT.T() - self.state.T()),
                        )
                        xbad.append(x)
                        ybad.append(y)
    ...
    def consistency_check_twophase(self):
        ...
        for q in np.linspace(0, 1, self.NQ_2phase):
            ...
            for T in np.linspace(
                Tmin, state.keyed_output(CP.iT_critical) - 1, self.NT_2phase
            ):
                ...
                try:
                    state.update(
                        pairkey,
                        self.state_QT.keyed_output(key1),
                        self.state_QT.keyed_output(key2),
                    )
                except ValueError as VE:
                    print("update_QT", T, q)
                    print(
                ...
                if _exception:
                    ...
                    # Check the error on the density
                    if (
                        abs(self.state_QT.rhomolar() / self.state.rhomolar() - 1) < 1e-3
                        and abs(self.state_QT.p() / self.state.p() - 1) < 1e-3
                        and abs(self.state_QT.T() - self.state.T()) < 1e-3
                    ):
                        xgood.append(x)
                        ygood.append(y)
                        if "REFPROP" not in self.backend:
                        ...
                        print(
                            ...
                            self.state_QT.T(),
                            "Drho:",
                            abs(self.state_QT.rhomolar() / self.state.rhomolar() - 1),
                            "DP",
                            abs(self.state_QT.p() / self.state.p() - 1),
                            "DT:",
                            abs(self.state_QT.T() - self.state.T()),
                        )
                        xbad.append(x)
                        ybad.append(y)
...
if __name__ == "__main__":
    ...
    for fluid in ["PROPANE"]:  # CP.__fluids__:
        ...
        skips = []
        ff = ConsistencyFigure(
            fluid, backend="REFPROP", additional_skips=skips
        )  # , NT_1phase = 10, Np_1phase = 10, NT_2phase = 100, NQ_2phase = 0)
        ff.add_to_pdf(PVT)
