...
def SimpleCycle(Ref, Te, Tc, DTsh, DTsc, eta_a, Ts_Ph="Ph", **kwargs):
    ...
    * skipPlot : If True, won't actually plot anything, just print COP
    ...
    for i in kwargs:
        warnings.warn(
            'This function has been deprecated, your input "{0}: {1}" will be ignored'.format(
                i, kwargs[i]
            ),
            DeprecationWarning,
        )
        ...
        for i in self._points:
            if keys:
                ...
                for j in self._points[i]:
                    label = u"{0:s} ({1:s})".format(
                        self.units[j].symbol, self.units[j].unit
                    )
            ...
            for j in self._points[i]:
                try:
                    row.append(
                        u"{0:11.3f}".format(self.units[j].from_SI(self._points[i][j]))
                    )
                except:
...
class BaseCycle(BasePlot):
    ...
    def cycle_states(self, value):
        if len(value) != self.STATECOUNT:
            raise ValueError(
                "Your number of states ({0:d}) is not in the list of allowed state counts: {1:s}.".format(
                    len(value), str(self.STATECOUNT)
                )
            )
    ...
    def system(self, value):
        if value in self.UNIT_SYSTEMS:
            ...
            raise ValueError(
                'Invalid unit_system input "{0:s}", expected a string from {1:s}'.format(
                    str(value), str(self.UNIT_SYSTEMS.keys())
                )
            )
    ...
    def fill_states(self, objs=None):
        ...
        for i in objs:
            ...
            if full:
                continue
            if (
                objs[i][CoolProp.iDmass] is not None
                and objs[i][CoolProp.iT] is not None
            ):
                self._state.update(
                    CoolProp.DmassT_INPUTS,
                    objs[i][CoolProp.iDmass],
                    objs[i][CoolProp.iT],
                )
            elif (
                objs[i][CoolProp.iP] is not None
                and objs[i][CoolProp.iHmass] is not None
            ):
                self._state.update(
                    CoolProp.HmassP_INPUTS,
                    objs[i][CoolProp.iHmass],
                    objs[i][CoolProp.iP],
                )
            elif (
                objs[i][CoolProp.iP] is not None
                and objs[i][CoolProp.iSmass] is not None
            ):
                self._state.update(
                    CoolProp.PSmass_INPUTS,
                    objs[i][CoolProp.iP],
                    objs[i][CoolProp.iSmass],
                )
            else:
    ...
    def state_change(self, in1, in2, start, ty1="lin", ty2="lin"):
        ...
        if end >= len(self.cycle_states):
            end -= len(self.cycle_states)
        ...
        for i, v in enumerate(inv):
            if typ[i] == "lin":
                ...
                val.append(
                    np.logspace(np.log10(start[v]), np.log10(end[v]), self.steps)
                )
            else:
