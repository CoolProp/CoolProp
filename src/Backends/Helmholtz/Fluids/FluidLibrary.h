...
class JSONFluidLibrary
{
   ...
   protected:
    ...
    // The block's shape is defined in exactly one place -- expression::compile_block()
    // -- so the fluid library and the standalone/scripting path can never drift.
    // The dump() round-trip is what keeps that single definition text-based (and so
    // nlohmann-free in the wrapper TUs); it runs once per expression block at fluid
    // load, and nlohmann round-trips doubles exactly.
    static inline CoolProp::ExpressionData parse_expression_block(const nlohmann::json& j, const std::string& fluidname) {
        CoolProp::expression::Program prog = CoolProp::expression::compile_block(j.dump(), "fluid " + fluidname);
   ...
   public:
    static void load_RES_transport_parameters(const nlohmann::json& res_json, const std::string& eos_key, CoolPropFluid& fluid,
        ...
        // The shipped table is generated by dev/convert_RES_csv_to_json.py with a fixed number of
        // coefficients per property, and the transport routines index those positions
        // unconditionally once `provided` is set.  Validate here as well as in the public setters:
        // this path never sees a user argument, but it does see a regenerated (or hand-edited)
        // res_transport_parameters.json, and a short list would otherwise be read out of bounds
        // at the first viscosity()/conductivity() call rather than rejected at load.
        auto require_size = [&](const std::vector<double>& v, std::size_t want, const char* what) {
            if (v.size() != want) {
                throw ValueError(format("RES transport parameters for %s (%s, %s): expected %d coefficients, got %d.", fluid.name.c_str(),
                                        eos_key.c_str(), what, static_cast<int>(want), static_cast<int>(v.size())));
            }
        };
        ...
        // ── viscosity ────────────────────────────────────────────────────────
        {
            ...
            if (!key.empty()) {
                ...
                require_size(d.n_dilute, RES_N_DILUTE, "dilute");
                ...
                if (fe.contains(eos_key)) {
                    ...
                    require_size(d.n_res, RES_N_RES_VISCOSITY, "residual");
        ...
        // ── thermal conductivity ─────────────────────────────────────────────
        {
            ...
            if (!key.empty()) {
                ...
                if (fe.contains(eos_key)) {
                    const nlohmann::json& eos = fe.at(eos_key);
                    d.n_res = eos.at("n").get<std::vector<double>>();
                    require_size(d.n_res, RES_N_RES_CONDUCTIVITY, "residual");
                    d.xita = eos.at("xita").get<double>();
                    d.group_num = eos.at("group").get<int>();
                    d.n_params_match_alpha = true;
                    d.provided = true;
