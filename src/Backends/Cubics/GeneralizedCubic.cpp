...
double AbstractCubic::d_am_term_dxi(double tau, const std::vector<double> &x, std::size_t itau, std::size_t i, bool xN_independent)
{
	if (xN_independent)
	{
		double summer = 2*x[i]*aij_term(tau, i, i, itau);
		for (int j = N-1; j >= 0; --j)
		{
			if (i == j){ continue; }
			summer += 2*x[j]*aij_term(tau, i, j, itau);
		}
		return summer;
	}
	else{
		...
		for (int k = N-2; k >= 0; --k)
		{
			summer += 2*x[k]*(aij_term(tau, i, k, itau)-aij_term(tau, k, N-1, itau));
		}
		return summer + 2*x[N-1]*(aij_term(tau, N-1, i, itau) - aij_term(tau, N-1, N-1, itau));
	}
}
...
double AbstractCubic::d2_psi_minus_dxidxj(double delta, const std::vector<double> &x, std::size_t itau, std::size_t idelta, std::size_t i, std::size_t j, bool xN_independent)
{
	...
	switch(idelta){
	case 0:
		return pow(delta*rho_r, 2)*db_dxi*db_dxj/pow(bracket, 2);
	case 1:
		return 2*delta*pow(rho_r, 2)*db_dxi*db_dxj/pow(bracket, 3);
	case 2:
		return 2*pow(rho_r,2)*db_dxi*db_dxj/pow(bracket, 4)*(2*delta*rho_r*b+1);
	case 3:
		return 12*pow(rho_r,3)*b*db_dxi*db_dxj/pow(bracket, 5)*(delta*rho_r*b+1);
	case 4:
		return 24*pow(rho_r,4)*pow(b, 2)*db_dxi*db_dxj/pow(bracket, 6)*(2*delta*rho_r*b + 3);
	default:
		throw -1;
	}
}
double AbstractCubic::PI_12(double delta, const std::vector<double> &x, std::size_t idelta)
{
	...
	switch(idelta){
	case 0:
		return (1+Delta_1*b*delta*rho_r)*(1+Delta_2*b*delta*rho_r);
	case 1:
...
double AbstractCubic::d_PI_12_dxi(double delta, const std::vector<double> &x, std::size_t idelta, std::size_t i, bool xN_independent)
{
	...
	switch(idelta){
	case 0:
		return delta*rho_r*db_dxi*(Delta_1*(1+Delta_2*b*delta*rho_r)+Delta_2*(1+Delta_1*b*delta*rho_r));
	case 1:
		return rho_r*db_dxi*(2*Delta_1*Delta_2*delta*rho_r*b + Delta_1*(1+Delta_2*b*delta*rho_r) + Delta_2*(1+Delta_1*b*delta*rho_r));
	case 2:
...
double AbstractCubic::d2_PI_12_dxidxj(double delta, const std::vector<double> &x, std::size_t idelta, std::size_t i, std::size_t j, bool xN_independent)
{
	...
	switch(idelta){
	case 0:
		return 2*Delta_1*Delta_2*pow(delta, 2)*pow(rho_r,2)*db_dxi*db_dxj;
	case 1:
		return 4*Delta_1*Delta_2*delta*pow(rho_r,2)*db_dxi*db_dxj;
	case 2:
		return 4*Delta_1*Delta_2*pow(rho_r,2)*db_dxi*db_dxj;
	case 3:
		return 0;
	case 4:
		return 0;
...
double AbstractCubic::psi_plus(double delta, const std::vector<double> &x, std::size_t idelta)
{
	...
	switch(idelta){
	case 0:
		return log((Delta_1*b*rho_r*delta+1)/(Delta_2*b*rho_r*delta+1))/(b*(Delta_1-Delta_2));
	case 1:
		return rho_r/PI_12(delta,x,0);
	case 2:
		return -rho_r*PI_12(delta,x,1)/pow(PI_12(delta,x,0),2);
	case 3:
		return rho_r*(-PI_12(delta,x,0)*PI_12(delta,x,2)+2*pow(PI_12(delta,x,1),2))/pow(PI_12(delta,x,0),3);
	case 4:
		return rho_r*(-PI_12(delta,x,0)*PI_12(delta,x,3) + 6*PI_12(delta,x,0)*PI_12(delta,x,1)*PI_12(delta,x,2) - 6*pow(PI_12(delta,x,1),3))/pow(PI_12(delta,x,0),4);
	default:
		throw -1;
	}
}
double AbstractCubic::d_psi_plus_dxi(double delta, const std::vector<double> &x, std::size_t idelta, std::size_t i, bool xN_independent)
{
	...
	if (idelta == 0){
		double b = bm_term(x);
		double db_dxi = d_bm_term_dxi(x, i, xN_independent);
		double paren = Delta_1*(Delta_2*delta*rho_r*b+1)-Delta_2*(Delta_1*delta*rho_r*b+1);
		return db_dxi/b*(delta*rho_r*paren/((Delta_1-Delta_2)*PI_12(delta,x,idelta)) - psi_plus(delta, x, idelta));
	}
	switch(idelta){
	case 1:
	...
	case 4:
		bracket = (d_PI_12_dxi(delta, x, 3, i, xN_independent)
			       + 2/rho_r*(3*PI_12(delta,x,1)*PI_12(delta,x,2) + PI_12(delta,x,0)*PI_12(delta,x,3))*d_psi_plus_dxi(delta, x,1,i,xN_independent)
...
double AbstractCubic::d2_psi_plus_dxidxj(double delta, const std::vector<double> &x, std::size_t idelta, std::size_t i, std::size_t j, bool xN_independent)
{
	...
	if (idelta == 0){
		double b = bm_term(x);
		double A = log((Delta_1*delta*rho_r*b+1)/(Delta_2*delta*rho_r*b+1));
		double dA_dxi = delta*rho_r*d_bm_term_dxi(x,i,xN_independent)*(Delta_1-Delta_2)/PI12;
		double dA_dxj = delta*rho_r*d_bm_term_dxi(x,j,xN_independent)*(Delta_1-Delta_2)/PI12;
		double d2A_dxidxj = delta*rho_r*(Delta_1-Delta_2)/pow(PI12, 2)*(PI12*d2_bm_term_dxidxj(x,i,j,xN_independent) 
			                                                         - d_PI_12_dxi(delta, x, 0, j,xN_independent)* d_bm_term_dxi(x,i,xN_independent));
		return 1/((Delta_1-Delta_2)*pow(b,3))*(-b*(A*d2_bm_term_dxidxj(x,i,j,xN_independent)
			                                      +dA_dxi*d_bm_term_dxi(x, j, xN_independent)
												  +dA_dxj*d_bm_term_dxi(x, i, xN_independent)
												  )
												  +2*A*d_bm_term_dxi(x, i, xN_independent)*d_bm_term_dxi(x, j, xN_independent)
												  +pow(b,2)*d2A_dxidxj);
	}
	switch(idelta){
	case 1:
	...
	case 3:{
		bracket = (rho_r*d2_PI_12_dxidxj(delta, x, 2, i, j, xN_independent) 
				   ...
				   + 4*(PI12*d_PI_12_dxi(delta, x, 1, j, xN_independent) + PI_12(delta, x, 1)*d_PI_12_dxi(delta, x, 0, j, xN_independent))*d_psi_plus_dxi(delta, x, 2, i, xN_independent)
	...
	case 4:
		bracket = (rho_r*d2_PI_12_dxidxj(delta, x, 3, i, j, xN_independent) 
			       + 6*(PI12*PI_12(delta, x, 2) + pow(PI_12(delta, x, 1), 2))*d2_psi_plus_dxidxj(delta, x, 2, i, j, xN_independent)
				   + 2*(PI12*PI_12(delta, x, 3) + 3*PI_12(delta, x, 1)*PI_12(delta, x, 2))*d2_psi_plus_dxidxj(delta, x, 1, i, j, xN_independent)
				   ...
				   + 2*(PI12*d_PI_12_dxi(delta, x, 3, j, xN_independent) 
				        + 3*PI_12(delta, x, 1)*d_PI_12_dxi(delta, x, 2, j, xN_independent)
						+ d_PI_12_dxi(delta, x, 0, j, xN_independent)*PI_12(delta, x, 3)
						+ 3*PI_12(delta, x, 2)*d_PI_12_dxi(delta, x, 1, j, xN_independent)
						)*d_psi_plus_dxi(delta, x, 1, i, xN_independent)
	...
	return -1/pow(PI_12(delta,x,0), 2)*bracket;
}
...
{
	return psi_minus(delta, x, itau, idelta)-1/(R_u*T_r)*tau_times_a(tau,x,itau)*psi_plus(delta,x,idelta);
}
