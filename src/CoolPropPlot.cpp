...
TEST_CASE("Check that the isolines are the same as from Python", "[Plot]") {
    ...
    }
    {
        ...
        const double expected_x[isoline_count][points_per_isoline] = {
            {75373.1268990847, 75410.9911120343, 75576.5817006842, 76301.4918515705, 79487.8877883423},
            {382785.230587562, 161389.442353423, 161516.218619847, 162076.984158624, 164637.062377748},
            {439466.649843278, 438148.172824183, 431912.066238713, 257605.319479605, 257512.839247251},
            {504550.626065609, 503783.529360534, 500331.593280549, 482707.17836027, 366958.520785585},
            {577604.594975215, 577097.065048066, 574850.152315666, 564443.789731478, 507875.800635261},
        };
        const double expected_y[isoline_count][points_per_isoline] = {
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
    ...
    }
    {
        ...
        CHECK_THAT(s_isolines[0].value, WithinAbs(426.0094838603979, 1e-10));
        CHECK_THAT(s_isolines[1].value, WithinAbs(925.275035741351, 1e-10));
        ...
        const double expected_x[isoline_count][points_per_isoline] = {
            {73758.1368335346, 73811.2861613466, 74043.6241898205, 75058.877171595, 79487.8877884639},
            {176257.349845373, 179794.80776157, 180290.319046323, 181487.967471083, 186690.959612256},
            {286286.175818423, 303984.726428754, 321692.362821626, 335551.688987583, 344087.839487745},
            {399372.560529411, 433400.354292331, 471964.896213688, 513835.931064389, 555824.663124966},
            {577604.594975109, 635258.237156208, 698999.445970919, 768745.631252129, std::nan("")},
        };
        const double expected_y[isoline_count][points_per_isoline] = {
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
    ...
    }
    {
        ...
        const double expected_x[isoline_count][points_per_isoline] = {
            ...
            {202365.843978259, 419230.112110974, std::nan(""), std::nan(""), std::nan("")},
            {142114.491283555, 204388.004478621, 351216.809706814, std::nan(""), std::nan("")},
            {133470.418481181, 172415.768780602, 235383.044874108, 357492.457483656, 669493.625997729},
            {70518.3287895171, 70601.2088976222, 70963.5807789925, 72548.3591970125, 79487.8877879114},
        };
        const double expected_y[isoline_count][points_per_isoline] = {
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
            {24999.9999999246, 109298.142136014, 477843.354976818, 2089095.63724655, 9133370.87584761},
...
TEST_CASE("Basic TS Plot has same output as Python", "[Plot]") {
    ...
    CHECK_THAT(plot.xaxis.min, WithinAbs(426.0094838603979, 1));
    ...
    }
    {
        ...
        CHECK_THAT(p_isolines[0].value, WithinAbs(24999.999999924625, 1e-8));
        CHECK_THAT(p_isolines[1].value, WithinAbs(109298.14213601447, 1e-8));
        CHECK_THAT(p_isolines[2].value, WithinAbs(477843.35497681826, 1e-8));
        CHECK_THAT(p_isolines[3].value, WithinAbs(2089095.6372465533, 1e-8));
        CHECK_THAT(p_isolines[4].value, WithinAbs(9133370.87584761, 1e-8));
        const double expected_x[isoline_count][points_per_isoline] = {
        ...
        const double expected_y[isoline_count][points_per_isoline] = {
            {171.786072659192, 220.381369310426, 220.381369310426, 265.362477224796, 454.999999999903},
            {171.798910666292, 248.745218249699, 248.745218249699, 308.633922577054, 506.387752763774},
            {171.854988815762, 258.195699077866, 287.4730373371, 355.964867192568, 560.293101202113},
            {172.099235421196, 258.742471436003, 342.561817261298, 411.323964493166, 618.036314177075},
    ...
    }
    {
        ...
        const double expected_y[isoline_count][points_per_isoline] = {
            ...
            {196.074550633908, 266.631159312275, std::nan(""), std::nan(""), std::nan("")},
            {213.664681842583, 299.984652703232, 301.726570477946, std::nan(""), std::nan("")},
            {228.411201679534, 322.843563825012, 426.787882130168, 331.521169967777, 328.042167528595},
            {241.568258023247, 341.661338916035, 458.593848045394, std::nan(""), 455.000000000079},
        };
    ...
    }
    {
        ...
        CHECK_THAT(d_isolines[4].value, WithinAbs(1593.2471569904417, 1e-10));
        const double expected_x[isoline_count][points_per_isoline] = {
            {524.173878302997, 1911.09303197673, 2092.95299735844, 2262.71394473455, 2423.07169138421},
            {448.103089615508, 1715.11956249481, 1932.46627813427, 2103.15612327654, 2263.90953791772},
            {437.189451893868, 972.489749676211, 1758.36241052056, 1935.7522861596, 2099.20643194095},
            {435.623706482598, 865.946977105694, 1292.02339683139, 1720.27746043057, 1899.38158004697},
