// EXPLORATORY / PROTOTYPE harness for the single-phase H,S flash research.
//
// This file holds the *prototype* HS solvers (continuation / isentrope /
// departure homotopy legs and their cascade) plus the [.]-hidden A/B
// characterization, benchmark and probe tests that exercise them.  None of this
// is the shipped flash -- the active production code lives in
// src/Backends/Helmholtz/FlashRoutines.cpp (hs_leg_*, hs_cascade), and the
// binding regression test of that active path lives in CoolProp-Tests-HS.cpp.
// Kept separate so review can focus on the active code.
//
...
HSResult solve_HS_departure(CoolProp::HelmholtzEOSMixtureBackend& AS, double h_t, double s_t) {
    ...
    auto lprops = [&](double T, double rho, double lam) -> LP {
        ...
        ++R.nevals;
        // One-shot: each calc_*_deriv_nocache computes the full derivative set and
        // returns a single component, so get both blocks once instead of 6-9x.
        const HelmholtzDerivatives a0d = AS.calc_all_alpha0_derivs_nocache(x, tau, delta, Tr, rhor);
        const HelmholtzDerivatives ard_all = AS.residual_helmholtz->all(AS, x, tau, delta, false);
